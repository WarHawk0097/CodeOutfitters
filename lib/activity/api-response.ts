// Same shape as lib/tasks/api-response.ts — duplicated per that file's own convention so each
// domain's error body type stays independent.
import "server-only";

export type ActivityApiErrorBody = {
  ok: false;
...
export function jsonError(
  ...
  const body: ActivityApiErrorBody = { ok: false, error: { code, message, ...(fields ? { fields } : {}) } };
