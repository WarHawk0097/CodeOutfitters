 ...
 *
 * JavaScript dashboard copy. This file is a 1:1 runtime port of
 * `workers/booking-reservation-worker.ts` (TypeScript) with all
 * TypeScript-only syntax stripped (interfaces, type annotations, type
 * aliases, generic return types, `as` casts). The Cloudflare Workers
 * dashboard editor parses pasted code as JavaScript, not TypeScript;
 * pasting the `.ts` source into the dashboard yields
 *   "Uncaught SyntaxError: Unexpected strict mode reserved word at worker.js:178"
 * (the `interface Env` declaration). This `.dashboard.js` file is
 * the dashboard-paste form. Deploy with `wrangler` (which accepts
 * `.ts`) using the original `.ts` source, or deploy from the dashboard
 * by pasting THIS `.dashboard.js` file.
 *
 * Purpose
 * -------
...
function originAllowed(requestOrigin, allowed) {
...
function validateBookingRequest(value) {
...
async function callReserveSlot(env, body) {
  ...
  if (code === 'P0001' && typeof message === 'string' && message.startsWith('slot_already_booked')) {
    return { ok: false, status: 409, error: 'slot_already_booked', tag: 'slot_already_booked' }
  }
  if (code === 'P0001' && typeof message === 'string' && message.startsWith('slot_not_found')) {
...
async function forwardToN8n(env, payload, origin) {
  ...
  try {
    const res = await fetch(env.N8N_BOOKING_WEBHOOK_URL, {
      ...
      body: JSON.stringify(Object.assign({}, payload, { type: 'booking' })),
    })
...
function notificationToField(n) {
...
export default {
  async fetch(request, env) {
    ...
    const body = { bookingId: rpcResult.bookingId }
