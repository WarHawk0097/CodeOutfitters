...
**This window (2026-08-16): live-mode SSR 500 regression found and fixed on `feat/leads-foundation-live`.** Root cause: `lib/demo/store.ts` built its seed state eagerly at module-import time (`createSeedState()`), and `useDemoState()` called `useSyncExternalStore` without checking `live` mode — both reachable from `app/dashboard/layout.tsx`'s `CommandCenterProvider`, which mounts on every dashboard page regardless of mode. In live mode this generated a full demo fixture set and `assertMockDataAllowed()` threw, producing a deterministic 500 on `/dashboard/pipeline` and `/dashboard/leads/[id]`. Fix: seed state is now built lazily on first actual access (`getSeedStateLazy()`), and `useDemoState()` reads `live` from `useCommandCenterConfig()` and swaps all three `useSyncExternalStore` arguments to a static `EMPTY_DEMO_STATE`/no-op subscribe when `live` is true — live mode never reaches `createSeedState()`/`generateLeads()`/`assertMockDataAllowed()`. `assertMockDataAllowed()` itself untouched. New regression suite `lib/demo/store.test.ts` (10 tests, including a source-surface check that every `useSyncExternalStore` argument is mode-gated, not just the snapshot getter) — empirically confirmed to fail 8/10 against the pre-fix code and pass 10/10 post-fix. Quality gate: `tsc --noEmit` clean, `eslint` clean, Vitest 2090/2090 (up from 2080, no regressions), `next build` clean. **Browser-verified this window** against the hosted QA workspace (Playwright, headless Chromium, real login): `/dashboard/pipeline` and `/dashboard/leads/06b6a526-57c3-4c8f-9d60-c1cd19992781` both return HTTP 200 with no `assertMockDataAllowed` error, no generic error page, and no browser console/page errors; the QA lead's card renders in the Pipeline board's Negotiation column and Lead Detail's status field agrees (both "Negotiation") — 12/12 checks passed. **Concurrency check inconclusive, not a confirmed pass or fail**: a gated single stale-`expectedStatus` PATCH attempt (expected to return 409 `stage_conflict` per the `change_lead_stage()` RPC contract documented above) instead returned an anomalous 422 on the first attempt and an anomalous 200 on an unintended second attempt (browser crash mid-check forced a retry outside the "one attempt" instruction). QA lead data (status/owner/history/activity counts) was verified unchanged after both. Static re-read of the full chain (`route.ts` → `LeadsPatchRequestSchema` → `server-provider.ts`'s `updateLead()`/`throwForStageRpcError()` → the `change_lead_stage()` migration SQL) found no logic defect — all code matches the documented, previously-audited contract. `free -h` showed severe swap-thrashing in this sandbox (14Gi/15Gi swap used) with PATCH requests taking 30s–2.2min server-side per the dev log, consistent with environmental resource duress rather than an application regression. Classified `HOSTED_PLATFORM_TRANSIENT` per the "do not repeatedly retry" instruction — a third live attempt was deliberately not made. This does not change row 2's `TESTED_LOCAL` status or the concurrency hardening already proven by the 8-case A-H PGlite matrix; it is a live-environment re-confirmation attempt that did not reach a clean result, not a new defect against the already-tested logic.
