...
| 4 | Transcript → CRM | TESTED_LOCAL | **Google Meet + Meeting Intelligence foundation (this window, 2026-08-21):** provider-neutral meetings domain (`lib/meetings/`) with Google Meet as first provider — `meetings`, `meeting_artifacts`, `transcripts`, `transcript_entries`, `ai_meeting_insights` (migration `20260820000000_meetings_transcripts.sql`), workspace-scoped RLS throughout, service-role-only sync writes. Transcript discovery/retrieval/normalization wired; migration is on hosted (objects + RLS/ACL verified read-only) but NOT recorded in `supabase_migrations.schema_migrations` (applied manually, not via `db push`). Not VERIFIED_END_TO_END: no controlled live Meet transcript ingested yet. |
