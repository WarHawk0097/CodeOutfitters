...
/**
 * Refuse to produce synthetic records in live mode.
 *
 * Synthetic fixtures are demo-only. The real boundary is the server-only
 * COMMAND_CENTER_MODE seam (lib/command-center/data.ts); this is a loud second
 * guard so a stray import can never emit invented rows as if they were customer
 * records once the production data tier is live.
 * ponytail: NODE_ENV=test also allowed so unit tests can generate freely.
 */
export function assertMockDataAllowed(): void {
  if (process.env.NODE_ENV === "test") return;
  if (getCommandCenterMode() !== "live") return;
  throw new Error(
    "Refusing to generate synthetic lead data in live mode: this data is " +
      "synthetic and must never be served as real records.",
  );
}
