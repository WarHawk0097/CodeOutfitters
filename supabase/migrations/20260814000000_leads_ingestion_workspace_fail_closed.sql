-- Closes LEAD_INGESTION_WORKSPACE_MISSING_FAIL_CLOSED (tracked in
-- docs/goals/CODEOUTFITTERS_MASTER_GOAL_STATUS.md).
--
-- 20260812020000_leads_workspace_ingestion_fix.sql made submit_inquiry() look
-- up the seeded 'codeoutfitters' workspace and assign it, but did not guard
-- against the lookup failing: if that workspace row is ever missing (renamed
-- slug, deleted, restore gap), v_workspace_id resolves to NULL and the
-- function silently inserts/merges leads with workspace_id = NULL again —
-- the exact defect the prior migration fixed, recreated one layer up.
--
-- This makes that lookup fail closed: no workspace found for slug
-- 'codeoutfitters' now aborts the whole call before any row is written,
-- instead of persisting an orphaned, permanently-invisible lead.
...
begin
  ...
  -- original result when the content matches; conflict when it differs. Checked
  -- before the fail-closed guard below: a replay of an already-successful
  -- submission must keep succeeding even if the workspace is later removed —
  ...
  -- Fail closed: a lead must never be created or merged without an owning
  -- workspace. Exactly one workspace is ever seeded (slug 'codeoutfitters',
  -- 20260729010000_owner_bootstrap.sql); if it is missing, this is a
  -- deployment misconfiguration, not a client error, and must not silently
  -- produce a NULL-workspace lead.
  if v_workspace_id is null then
    raise exception 'inquiry_workspace_missing'
      using errcode = 'P0001',
