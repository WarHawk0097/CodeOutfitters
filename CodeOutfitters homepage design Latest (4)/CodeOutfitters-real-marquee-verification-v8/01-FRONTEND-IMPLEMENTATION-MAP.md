...
1. **Homepage integrations** is no longer `integrations-marquee`. New component: `sections/home/integrations-hub.tsx` — central hub node + 12 absolute-positioned tool nodes over an SVG connection layer (desktop) and a hub-banner + tile grid (≤860px). Source 21st patterns: `integration-hero.tsx` (content stack + tool tiles) + `integrations-section.tsx` (tile field fallback).
2. **FAQ**: `components/ui/accordion.tsx` now renders the `faq-accordion.tsx` mechanics faithfully — numbered marker left, circular rotating chevron right, in-card answer, grid-rows height animation. Used on Home, Services, Industries, Process, Security, Contact.
3. **Security integrations**: new `sections/security/security-architecture.tsx` replaces `integrations-marquee.tsx` — grouped tool cards (Business/Commerce/Communication/Scheduling, scope tag per tool) connected through shield checkpoints to a central control-layer hub.
4. **Navigation**: 7 items (Home, Services, Industries, Process, Case Studies, About, Contact) on all pages + far-right Book a Call CTA → /contact. Mobile: hamburger menu (all 7 + CTA), no clipping.


---

## v5 REPAIR ADDENDUM (exact FAQ + logo slider components)

1. **Homepage integrations** → `components/ui/marquee-logo-scroller.tsx` (MarqueeLogoScroller). Bordered shell, two-col [3fr/2fr] header, masked viewport, flex `w-max` track `translateX(0→-50%)`, logos ×2, pause-on-hover, per-card gradient+lift, speed 40s. Data: 12 tools (n8n…Calendly). Replaces the v3 hub.
2. **Security integrations** → same MarqueeLogoScroller (`components/sections/security/secure-marquee.tsx`) wrapped with a security control band (Scoped access / Server-side secrets / Human approval / Activity logging / Rollback path) + per-card group tags + darker palette + checkpoint pulses; speed 44s. Replaces the v3 architecture grid.
3. **FAQ everywhere** → `components/ui/faq-accordion-block.tsx` (FAQAccordionBlock): Badge header + title + description, single-open `openIndex`, height+opacity `AnimatePresence`, rotating chevron, same-card answer under divider, hover border/shadow, per-page eyebrow/title, optional bottom CTA (Home + Services). Used on Home, Services, Industries, Process, Security, Contact.
4. **Reduced motion:** `.marquee-track`/`.mq-track-v5` → no animation, wrap, width 100%, dup hidden; `.faq-motion` → transitions ~0ms.


---

## v6 FIDELITY ADDENDUM
- **MarqueeLogoScroller** (homepage + security) now uses **real logo SVGs** in `assets/logos/` inside **large logo cards** (home 208×112, security 212×120), logo-dominant, faithful to the supplied component geometry (bordered shell, [3fr_2fr] header + separator, masked twice-rendered track, pause-on-hover, gradient/lift). Security adds a control band + per-card group tags + darker palette.
- **FAQ** is now the **dark** FAQAccordionBlock everywhere (near-black emerald section, dark charcoal cards, thin borders, small chevron, same-card expansion, dark CTA card on Home + Services). Ivory FAQ fully removed.
